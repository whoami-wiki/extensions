...
This is a personal encyclopedia documenting the wiki owner's life through wiki pages. The wiki owner's details can be found in the [[Me]] page — read it to learn their name and use it when referring to them instead of saying "the wiki owner" or similar.
...
This project ships dedicated OpenCode agents and skills for working on the wiki:

| Kind | Name | Use |
|------|------|-----|
| Agent (primary) | `editor` | Default mode — research sources, draft pages, maintain talk pages |
| Agent (subagent) | `curator` | Nightly main-page curation. Invoke via `@curator` or schedule with cron |
| Skill | `editorial-guide` | Page conventions, words-to-watch, citation templates, talk page structure |

The editor is the default primary mode. Switch to it (or stay in it) for most wiki work; the editorial-guide skill is loaded automatically when needed. For nightly main-page curation, run `@curator`.

## Sources

Sources can be listed with `wai source list`, which returns all pages in the wiki's source namespace. Source pages have information about different primary sources of data available that can be used for editorial purposes. Each source page has a unique snapshot id in the infobox that can be used to look up their info in the vault.
...
4. Do the work (follow the editor agent's workflow)
...
See the `editor` agent for the full research → draft → publish workflow, and the `editorial-guide` skill for page conventions, prose discipline, and citation templates.
